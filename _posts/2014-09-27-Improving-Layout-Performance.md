...
> 目录：
> 
> 1. [优化Layout的层级](#anchor_1)——教你使用SDK自带的工具来检查Layout的性能瓶颈
> 2. [重用Layout](#anchor_2)——教你创建高效、可重用的Layout
> 3. [按需载入Layout](#anchor_3)——教你如何分步载入Layout来提高初始性能

Layout是Android应用中直接影响用户体验的关键部分，如果实现得不好，你的UI可能会出现卡顿。Android SDK提供了帮助你找到Layout性能瓶颈的工具，本节将会介绍如何使用这些工具，消耗小量的内存来实现流畅的UI。
...
**解决方法**：
首先通过`<include>`标签的id获取对应的的Layout，然后再通过Layout中的id获取的其中的`View`。代码示例如下：
